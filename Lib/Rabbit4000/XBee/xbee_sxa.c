	...
	void FAR										*base)
{
   ...
   */
   sxa_node_t FAR *sxa = base;
   // Ignore ZigBee node type in version field
	uint16_t zbVer = (uint16_t)sxa->firmware_version & 0xF0FF;
   uint16_t zbV = zbVer & 0xF000;
...
_sxa_disc_process_node_data             <xbee_sxa.c>
...
PARAMETER1:  xbee - device that received the message
PARAMETER2:  raw - pointer to the message
PARAMETER3:  length - length of the message


RETURNS:  0        - successfully parsed
          !0	error parsing data or calling <handler>()
...
_sxa_disc_atnd_response                 <xbee_sxa.c>
...
/*** EndHeader */
/* START FUNCTION DESCRIPTION ********************************************
_sxa_disc_handle_frame_0x95             <xbee_sxa.c>
...
/*** EndHeader */
/* START _FUNCTION DESCRIPTION *******************************************
_sxa_disc_cluster_handler               <xbee_sxa.c>
...
/*** EndHeader */
/* START _FUNCTION DESCRIPTION *******************************************
_sxa_io_process_response                <xbee_sxa.c>
...
PARAMETER1:  xbee - device that received the message
PARAMETER2:  raw - pointer to the message (starting at sample count field)
PARAMETER3:  length - length of the message
										...
										uint16_t length, void FAR *context)
{
   ...
   if (resp->header.status == XBEE_AT_RESP_SUCCESS)
   {
	...
	void FAR										*base)
{
   ...
   if ((response->flags & XBEE_CMD_RESP_MASK_STATUS)
                                             != XBEE_AT_RESP_SUCCESS)
	{
      ...
      c = (sxa_cached_t FAR *)(
      			(char FAR *)base + (reg->offset - _SXA_CACHED_PREFIX_SIZE)
               					);
                     ...
                     uint16_t cache_group)
{
   ...
   {
   	c = (sxa_cached_t FAR *)((char FAR *)sxa + zb->sxa_offs);
   	return c->flags;
   }
   ...
   return *(sxa_cache_flags_t FAR *)((char FAR *)sxa + group->flags_offs);
}
                     ...
                     sxa_cache_flags_t flags)
{
   ...
   group = _sxa_cache_group_by_id(cache_group);
   if (group)
		*(sxa_cache_flags_t FAR *)((char FAR *)sxa + group->flags_offs) = flags;
}
