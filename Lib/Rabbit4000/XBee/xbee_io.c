...
xbee_io_response_parse                  <xbee_io.c>
...
PARAMETER1:  parsed - Result structure
...
/* START FUNCTION DESCRIPTION ********************************************
xbee_io_response_dump                   <xbee_io.c>
...
PARAMETER1:  io - pointer to an xbee_io_t structure as set up by
...
/*** EndHeader */
/* START FUNCTION DESCRIPTION ********************************************
xbee_io_get_digital_input               <xbee_io.c>
...
PARAMETER1:  io - pointer to an xbee_io_t structure as set up by
              xbee_io_response_parse()
PARAMETER2:  index - digital input number e.g. 0 for DIO0, 12 for DIO12.
...
/*** EndHeader */
/* START FUNCTION DESCRIPTION ********************************************
xbee_io_get_digital_output              <xbee_io.c>
...
PARAMETER1:  io - pointer to an xbee_io_t structure as set up by
              xbee_io_query()
PARAMETER2:  index - digital output number e.g. 0 for DIO0, 2 for DIO2.

RETURNS:  0        - Output low
          1        - Output high
          -EINVAL  - 
...
/*** EndHeader */
/* START FUNCTION DESCRIPTION ********************************************
xbee_io_get_analog_input                <xbee_io.c>
...
PARAMETER1:  io - pointer to an xbee_io_t structure as set up by
              xbee_io_response_parse()
PARAMETER2:  index - analog input number e.g. 0 for AD0, 2 for AD2,
              7 for Vcc reading (where available)

RETURNS:  0        - ..32767 raw single-ended analog reading, normalized to the
          ...
          -16384   - ..16383 raw differential analog reading, normalized to the
                     range -16384..16383.  This is reserved for future hardware.
          XBEE_IO_ANALOG_INVALID
...
/*** EndHeader */
/* START FUNCTION DESCRIPTION ********************************************
xbee_io_set_digital_output              <xbee_io.c>
...
PARAMETER1:  xbee - local device through which to action the request
PARAMETER2:  io - pointer to an xbee_io_t structure as set up by
              xbee_io_query().  Shadow state of I/O
              may be modified
PARAMETER3:  index - digital output number e.g. 0 for DIO0, 2 for DIO2.
PARAMETER4:  state - new state of I/O.  If the XBEE_IO_FORCE flag is
...
PARAMETER5:  address - NULL for local XBee, or destination IEEE (64-bit)
              or network (16 bit) address of a remote device.  See
              <wpan_envelope_create>() for use of IEEE and network
              addressing.
...
DESCRIPTION:
     Configure XBee digital and analog IOs.
...
PARAMETER2:  io - pointer to an xbee_io_t structure as set up by
              xbee_io_query().  Shadow state of I/O
              may be modified
PARAMETER3:  index - digital or analog I/O number e.g. 0 for DIO0 or AD0.
PARAMETER4:  type - type of I/O to configure.  If the XBEE_IO_FORCE flag is
...
DESCRIPTION:
     Configure XBee automatic IO sampling options.
...
PARAMETER2:  io - pointer to an xbee_io_t structure as set up by
              xbee_io_query().  Shadow state of I/O
              may be modified
PARAMETER3:  sample_rate - sample period in ms.  0 to turn off sampling,
              otherwise must be a value geater than
              XBEE_IO_MIN_SAMPLE_PERIOD (50 ms with current hardware).
PARAMETER4:  change_mask - bitmask of I/Os which are to generate samples when
...
/*** EndHeader */
/* START FUNCTION DESCRIPTION ********************************************
_xbee_io_query_handle_pr                <xbee_io.c>

SYNTAX:
   void _xbee_io_query_handle_pr(  const xbee_cmd_response_t FAR *response, 
                                  const struct xbee_atcmd_reg_t FAR *reg, 
                                  void FAR *base )

DESCRIPTION:
     Read current configuration of XBee digital and analog IOs.
...
PARAMETER1:  xbee - local device through which to action the request
PARAMETER2:  io - pointer to an xbee_io_t structure, which will be
              modified by this call.  Since the results are
              returned asynchronously, the data pointed to must
              be static.
PARAMETER3:  address - NULL for local XBee, or destination IEEE (64-bit)
              or network (16 bit) address of a remote device.
...
/*** EndHeader */
/* START FUNCTION DESCRIPTION ********************************************
xbee_io_query_status                    <xbee_io.c>
...
PARAMETER1:  io - I/O query to check (pointer as passed to
              xbee_io_query()).
