...
enum {
	XBEE_FW_STATE_INIT = 0,			// initial state
...
xbee_fw_install_init                    <xbee_firmware.c>
...
PARAMETER1:  xbee - XBee device to install firmware on.  Must have been
              set up with xbee_dev_init().

PARAMETER2:  target - The current version of this library can only update the
...
PARAMETER3:  source - Structure with function pointers for seeking and reading
...
RETURNS:  0        - success
          ...
          -EIO     - XBee device passed to function doesn't have a
...
/*** EndHeader */
/* START FUNCTION DESCRIPTION ********************************************
xbee_fw_install_ebl_state               <xbee_firmware.c>
...
PARAMETER1:  source - object used to track state of transfer


RETURNS:  unique value that changes whenever the transfer state changes
...
/*** EndHeader */
/* START FUNCTION DESCRIPTION ********************************************
xbee_fw_install_ebl_tick                <xbee_firmware.c>
...
RETURNS:  0        - update is in progress
...
int xbee_fw_install_ebl_tick( xbee_fw_source_t *source)
{
	...
	switch (source->state)
	{
		...
		case XBEE_FW_STATE_RESET:
	      ...
	      // close serial port
	      #ifdef XBEE_FIRMWARE_VERBOSE
	         printf( "%s: close serial port\n", __FUNCTION__);
	      #endif

	      // Open serial port to initialize Tx/Rx and RTS/CTS pins correctly,
	      // but then close it so we can control those pins directly.

	      xbee_ser_open( serport, 115200);
	      xbee_ser_flowcontrol( serport, 1);

	      // DEVNOTE: Might not be able to control RTS on Win32 after closing
	      //				the serial port.
//	      xbee_ser_close( serport);
		...
		case XBEE_FW_STATE_BREAK:
			...
			if (xbee_millisecond_timer() - source->timer > 150)
			{
	         #ifdef XBEE_FIRMWARE_VERBOSE
	            printf( "%s: reopen serial port and assert RTS\n",
	            	__FUNCTION__);
	         #endif

	         // re-open serial port at 115200 bps & clear break condition
	         xbee_ser_open( serport, 115200);
		...
		case XBEE_FW_STATE_XMODEM_DONE:
			if (xbee_millisecond_timer() - source->timer < 3000)
			{
				return 0;
			}
			xbee_ser_putchar( serport, '\r');
	...
	if (xbee_millisecond_timer() - source->timer > XBEE_FW_LOAD_TIMEOUT)
	{
		source->tries += 1;
...
/*** EndHeader */
/* START FUNCTION DESCRIPTION ********************************************
xbee_fw_status_ebl                      <xbee_firmware.c>
...
PARAMETER1:  source - State variable to generate status string for.

PARAMETER2:  buffer - Buffer (at least 80 bytes) to receive dynamic status
              string.


RETURNS:  Returns parameter 2 or a pointer to a fixed status string.
...
typedef struct _xbee_oem_header
{
...
_xbee_oem_verify                        <xbee_firmware.c>

SYNTAX:
   int _xbee_oem_verify( xbee_fw_source_t *source)

DESCRIPTION:
     Validate the firmware image stored in <source>.  Checks the
     header, length and checksum.


PARAMETER1:  source - Firmware source initialized with \c seek and \c read
              functions.


RETURNS:  0        - Source contains a valid firmware image.
...
#endif
/* START FUNCTION DESCRIPTION ********************************************
xbee_fw_install_oem_tick                <xbee_firmware.c>
...
PARAMETER1:  source - Firmware source initialized with seek and read
              functions.


RETURNS:  1        - Successfully installed new firmware.
...
int xbee_fw_install_oem_tick( xbee_fw_source_t *source)
{
	...
	switch (source->state)
	{
		...
		case XBEE_FW_STATE_BREAK:		// 400ms break and then we're in bootloader
			if (xbee_millisecond_timer() - source->timer < 400)
			{
				break;
			}
		...
		case XBEE_FW_STATE_TX_START:
			...
			if ((ch == -ENODATA)
				&& (xbee_millisecond_timer() - source->timer < 1000))
			{
				break;
			}
			#ifdef XBEE_FIRMWARE_VERBOSE
		...
		case XBEE_FW_STATE_RX_BLOCK:
			...
			if (xbee_millisecond_timer() - source->timer < 4000)
			{
				break;
			}
...
/*** EndHeader */
/* START FUNCTION DESCRIPTION ********************************************
xbee_fw_status_oem                      <xbee_firmware.c>
...
/*** EndHeader */
/* START _FUNCTION DESCRIPTION *******************************************
_xbee_fw_buffer_seek                    <xbee_firmware.c>
...
PARAMETER1:  context - address of xbee_fw_buffer_t object
PARAMETER2:  offset - offset to new position


RETURNS:  -EINVAL  - offset greater than length of file
          0        - seek successful
...
/* START _FUNCTION DESCRIPTION *******************************************
_xbee_fw_buffer_read                    <xbee_firmware.c>

SYNTAX:
   int _xbee_fw_buffer_read( void FAR *context,  void FAR *buffer, 
                             int16_t bytes)
...
PARAMETER1:  context - address of xbee_fw_buffer_t object
PARAMETER2:  buffer - destination buffer to read bytes into
PARAMETER3:  bytes - number of bytes to read


RETURNS:  -ENODATA - no more bytes available to read
...
/* START FUNCTION DESCRIPTION ********************************************
xbee_fw_buffer_init                     <xbee_firmware.c>
...
PARAMETER1:  fw - structure to configure for reading firmware from a buffer
PARAMETER2:  length - number of bytes in firmware image
PARAMETER3:  address - address of buffer containing firmware
