...
RETURNS:  >0       - number of requests expired
...
_xbee_cmd_handle_to_address             <xbee_atcmd.c>
...
PARAMETER1:  handle - Handle returned from xbee_cmd_create().


RETURNS:  Pointer into the request table or NULL if handle is invalid.

SEE ALSO:  xbee_cmd_create()
...
/*** EndHeader */
/* START _FUNCTION DESCRIPTION *******************************************
_xbee_cmd_issue_list                    <xbee_atcmd.c>
   ...
   else
   {
	...
	switch (reg->type)
	{
	   ...
	   case XBEE_CLT_SET_STR:
	      xbee_cmd_set_param_bytes( request, devptr,
	      	(uint8_t) strlen( (char FAR *)devptr));
	      break;
	   case XBEE_CLT_SET_BE:
	      switch (reg->bytes)
	      {
	         default:
	         case 1:
	            xbee_cmd_set_param( request, *(uint8_t FAR *)devptr);
	            break;

	         case 2:
	            xbee_cmd_set_param( request, *(uint16_t FAR *)devptr);
	            break;

	         case 4:
	            xbee_cmd_set_param( request, *(uint32_t FAR *)devptr);
	            break;
	      }
	      break;
	}
...
/* START FUNCTION DESCRIPTION ********************************************
xbee_cmd_list_execute                   <xbee_atcmd.c>
...
PARAMETER1:  xbee - Device to execute commands
PARAMETER2:  clc - List head to set up.  This must be static since callback
              functions access it asynchronously.
PARAMETER3:  list - First entry of list of commands to execute.  The list
              must be termineted by either #XBEE_ATCMD_REG_END or
              #XBEE_ATCMD_REG_END_CB.  List entries are created using
              #XBEE_ATCMD_REG macros etc.
PARAMETER4:  base - Base address of a structure to fill in with command
              results or as a source of values to set.
PARAMETER5:  address - Remote address, or NULL if local device.


RETURNS:  0        - started sending commands from the list
          -ENOSPC  - the AT command table is full (increase the compile-time
                     macro XBEE_CMD_REQUEST_TABLESIZE)
          -EINVAL  - an invalid parameter was passed to the function

SEE ALSO:  xbee_cmd_list_status()
...
/*** EndHeader */
/* START FUNCTION DESCRIPTION ********************************************
xbee_cmd_list_status                    <xbee_atcmd.c>
...
PARAMETER1:  clc - List head passed to xbee_cmd_list_execute().


RETURNS:  XBEE_COMMAND_LIST_RUNNING currently executing commands
...
SEE ALSO:  xbee_cmd_list_execute()
...
/*** EndHeader */
/* START FUNCTION DESCRIPTION ********************************************
xbee_cmd_init_device                    <xbee_atcmd.c>

SYNTAX:
   int xbee_cmd_init_device( xbee_dev_t *xbee)

DESCRIPTION:
     Enable AT Command Response handlers for XBee device <xbee>.


PARAMETER1:  xbee - XBee device to use as the target (local) or to send
              through (remote).  This function will automatically call
              xbee_cmd_query_device if it hasn't already been called for this
              device.


RETURNS:  0        - the XBee device was successfully configured to send and
                     receive AT commands
          -EINVAL  - an invalid parameter was passed to the function


SEE ALSO:  xbee_cmd_query_device(), xbee_cmd_create(),
...
int xbee_cmd_init_device( xbee_dev_t *xbee)
{
	...
	// Not requesting a command handle, actually initializing the AT command
	// layer for this XBee device.
...
/* START FUNCTION DESCRIPTION ********************************************
xbee_cmd_query_device                   <xbee_atcmd.c>
...
PARAMETER1:  xbee - XBee device to query.
PARAMETER2:  refresh - if non-zero, just refresh the volatile values
              (e.g., network settings, as opposed to device
              serial number)


RETURNS:  0        - Started querying device.
          -EBUSY   - Transmit serial buffer is full, or XBee is not accepting
                     serial data (deasserting /CTS signal).

SEE ALSO:  xbee_cmd_init_device(), xbee_cmd_query_status()
...
/* START FUNCTION DESCRIPTION ********************************************
xbee_cmd_query_status                   <xbee_atcmd.c>
...
PARAMETER1:  xbee - device to check


RETURNS:  0        - query completed
          -EINVAL  - <xbee> is NULL
          -EBUSY   - query underway
          -ETIMEDOUT - query timed out
          -EIO     - halted, but query may not have completed
                     (unexpected response)
...
PARAMETER2:  command - Two-letter AT Command to send
              (e.g., "VR", "NI", etc.).


RETURNS:  >0       - a "handle" to a request that can be built up and sent
                     to a local (serially-attached) or remote XBee module.
          -ENOSPC  - the AT command table is full (increase the compile-time
                     macro XBEE_CMD_REQUEST_TABLESIZE)
          -EINVAL  - an invalid parameter was passed to the function

SEE ALSO:  xbee_cmd_init_device(), xbee_cmd_query_device(),
...
/*** EndHeader */
/* START _FUNCTION DESCRIPTION *******************************************
_xbee_cmd_release_request               <xbee_atcmd.c>
...
PARAMETER1:  request - Address of the table entry.


RETURNS:  0        - request freed
          -EINVAL  - address (<request>) is not valid

SEE ALSO:  xbee_cmd_create(), xbee_cmd_release_handle()
...
/*** EndHeader */
/* START FUNCTION DESCRIPTION ********************************************
xbee_cmd_release_handle                 <xbee_atcmd.c>
...
PARAMETER1:  handle - handle to the request (as returned by xbee_cmd_create)
...
/*** EndHeader */
/* START FUNCTION DESCRIPTION ********************************************
xbee_cmd_set_command                    <xbee_atcmd.c>
...
PARAMETER2:  command - two-letter AT Command to send (e.g., "VR", "NI", etc.)


RETURNS:  0        - command changed
          -EINVAL  - <handle> is not valid

SEE ALSO:  xbee_cmd_create(), xbee_cmd_set_callback(), xbee_cmd_set_target(),
          xbee_cmd_set_param(), xbee_cmd_set_param_bytes(),
          xbee_cmd_set_param_str(), xbee_cmd_send()
...
/*** EndHeader */
/* START FUNCTION DESCRIPTION ********************************************
xbee_cmd_set_callback                   <xbee_atcmd.c>
...
RETURNS:  0        - callback set
          -EINVAL  - <handle> is not valid

SEE ALSO:  xbee_cmd_create(), xbee_cmd_set_command(), xbee_cmd_set_target(),
          xbee_cmd_set_param(), xbee_cmd_set_param_bytes(),
          xbee_cmd_set_param_str(), xbee_cmd_send()
...
/*** EndHeader */
/* START FUNCTION DESCRIPTION ********************************************
xbee_cmd_set_target                     <xbee_atcmd.c>
...
RETURNS:  0        - target set
          -EINVAL  - <handle> is not valid
          -ENOSYS  - function not implemented (XBEE_CMD_DISABLE_REMOTE defined)

SEE ALSO:  xbee_cmd_create(), xbee_cmd_set_command(), xbee_cmd_set_callback(),
          xbee_cmd_set_param(), xbee_cmd_set_param_bytes(),
          xbee_cmd_set_param_str(), xbee_cmd_send()
...
/*** EndHeader */
/* START _FUNCTION DESCRIPTION *******************************************
_xbee_cmd_encode_param                  <xbee_atcmd.c>
...
PARAMETER1:  buffer - buffer to store value
PARAMETER2:  value - value to store


RETURNS:  Number of bytes required (1, 2 or 4) to hold value.
...
/*** EndHeader */
/* START FUNCTION DESCRIPTION ********************************************
xbee_cmd_set_flags                      <xbee_atcmd.c>
...
PARAMETER1:  handle - 
              Handle to the request, as returned by xbee_cmd_create().
PARAMETER2:  flags - One or more of the following:
...
/*** EndHeader */
/* START FUNCTION DESCRIPTION ********************************************
xbee_cmd_clear_flags                    <xbee_atcmd.c>
...
RETURNS:  0        - flags cleared
          -EINVAL  - <handle> is not valid

SEE ALSO:  xbee_cmd_set_flags
...
/*** EndHeader */
/* START FUNCTION DESCRIPTION ********************************************
xbee_cmd_set_param                      <xbee_atcmd.c>
...
/*** EndHeader */
/* START FUNCTION DESCRIPTION ********************************************
xbee_cmd_set_param_bytes                <xbee_atcmd.c>
...
PARAMETER2:  data - Pointer to bytes (MSB-first) to copy into request.

PARAMETER3:  length - Number of bytes to copy.
              0 < <length> <= #XBEE_CMD_MAX_PARAM_LENGTH
...
/*** EndHeader */
/* START FUNCTION DESCRIPTION ********************************************
xbee_cmd_set_param_str                  <xbee_atcmd.c>
...
RETURNS:  0        - parameter set
          -EINVAL  - <handle> is not valid
          -EMSGSIZE - <str> is more than #XBEE_CMD_MAX_PARAM_LENGTH
                     characters.
...
#endif
/* START _FUNCTION DESCRIPTION *******************************************
_xbee_cmd_handle_response               <xbee_atcmd.c>
	...
	uint16_t length, void FAR *context)
{
	...
	const xbee_frame_at_response_t FAR *frame;
	...
	frame = (const xbee_frame_at_response_t FAR *)rawframe;
	...
	else
	{
      ...
      else
      {
		...
		if (0 == (response.value_length = value_length))
		{
			response.value = 0;
		}
		else
...
/*** EndHeader */
/* START _FUNCTION DESCRIPTION *******************************************
_xbee_cmd_modem_status                  <xbee_atcmd.c>
...
PARAMETER1:  xbee - XBee device to use as the target.
...
PARAMETER3:  data - Optional big-endian (MSB-first) value to assign to
              <command>.  Use NULL if <command> doesn't take a
              parameter.

PARAMETER4:  length - Number of bytes in <data>; ignored if <data> is NULL.
