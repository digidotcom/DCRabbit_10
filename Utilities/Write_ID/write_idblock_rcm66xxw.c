...
*/
/*
	write_idblock_rcm66xxw.c

	Example of using WriteIdBlock.lib and WriteIdBlock_RCM66xxW.lib to write a
	system ID Block to an RCM66xxW board.

	Each RCM66xxW family board's WiFi is individually calibrated. The WiFi
	...
	To program an RCM66xxW board which lacks an existing correct and valid system
...
void main(void)
{
...
#if RCM6600W == _BOARD_TYPE_
	// add RCM6600W specific, fixed information
	wib_addRCM6600WInfo(build);
#elif RCM6650W == _BOARD_TYPE_
	// add RCM6650W specific, fixed information
	wib_addRCM6650WInfo(build);
#else
	...
	/*
		...
		For each RCM66xxW family board, the test fixture should provide individual
