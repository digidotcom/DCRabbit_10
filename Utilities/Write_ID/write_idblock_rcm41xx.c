...
*/
/*
	write_idblock_rcm41xx.c

	Example of using WriteIdBlock.lib and WriteIdBlock_RCM41xx.LIB to write a
	system ID Block to an RCM41xx board.

	To program an RCM41xx board which lacks an existing correct and valid system
...
#if RCM4100 == _BOARD_TYPE_
	// add RCM4100 specific, fixed information
	wib_addRCM4100Info(build);
#elif RCM4110 == _BOARD_TYPE_
	// add RCM4110 specific, fixed information
	wib_addRCM4110Info(build);
#elif RCM4120 == _BOARD_TYPE_
	// add RCM4120 specific, fixed information
	wib_addRCM4120Info(build);
#else	// RCM4100 == _BOARD_TYPE_
	#fatal "This sample does not support the current _BOARD_TYPE_ definition."
#endif	// RCM4100 == _BOARD_TYPE_
