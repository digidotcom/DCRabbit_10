...
*/
/*
	write_idblock_rcm57xx.c

	Example of using WriteIdBlock.lib and WriteIdBlock_RCM57xx.LIB to write a
	system ID Block to an RCM57xx board.

	To program an RCM57xx board which lacks an existing correct and valid system
	...
	For an RCM5700/10 add:
	   _BOARD_TYPE_=RCM5700
	   _DC_CLK_DBL_=1
	   _DC_BRD_OPT0_=0x20
	   _DC_MD0_=0x1
	   _DC_MD0_ID_=0x1DA
...
#if RCM5700 == _BOARD_TYPE_
	// add RCM5700 specific, fixed information
	wib_addRCM5700Info(build);
#elif RCM5750 == _BOARD_TYPE_
	// add RCM5750 specific, fixed information
	wib_addRCM5750Info(build);
#else
	#fatal "This sample does not support the current _BOARD_TYPE_ definition."
#endif	// RCM5700 == _BOARD_TYPE_
