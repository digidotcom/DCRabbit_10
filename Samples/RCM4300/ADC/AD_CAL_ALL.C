...
main ()
{
	...
	while (1)
	{
		...
		printf("\nAdjust to just above lower voltage of %s and enter actual =  ",
                vstr[gaincode]);
		gets(buffer);
		...
		printf("\nAdjust to just below higher voltage of %s and enter actual =  ",
                 vstr[gaincode]);
		gets(buffer);
		for (channel=STARTCHAN; channel<=ENDCHAN; channel++)
		{
		...
		for (channel=STARTCHAN; channel<=ENDCHAN; channel++)
		{
 			anaInCalib(channel, SINGLE, gaincode, ln[channel].value1,
                     ln[channel].volts1,ln[channel].value2, ln[channel].volts2);
		}
		...
		while (strcmp(buffer,"q") && strcmp(buffer,"Q"))
		{
			...
			printf("press enter key to read values again");
         printf(" or 'Q' to calibrate another gain\n\n");
			gets(buffer);
		}
	}
}
