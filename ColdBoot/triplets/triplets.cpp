...
void usage()
{
   cerr << "This is the triplet generator utility for Rabbit processor cold loaders.\n\n";
   cerr << "Format: triplets <source_name> <output_name>\n";
   cerr << "It takes the name of the source file as input without the extension\n";
   cerr << "and assumes that both the .C file and .bin file exist. Likewise, do\n";
   cerr << "not include an extension on the second parameter; this utility appends\n";
   cerr << "a .bin to the output.\n";
}
...
int main(int argc, char * argv[])
{
   ...
   ofstream bin_output_file(bin_output_name.c_str(), ios::binary);
   if (bin_output_file.fail()) {
      cerr << "I couldn't open " << bin_output_name << endl;
      exit(4);
   }
   ...
   while (command != "__EOF") {
      ...
      if (io_reg == "start") {
         ...
         unsigned short io_address = io_map[io_reg];
         // This is the Rabbit specific way to specify an io register in the
         // cold loader triplets.
         io_address |= 0x8000;
         const char * fake_byte = reinterpret_cast<const char *>(&io_address);
         // THIS IS NOT PORTABLE!
         bin_output_file.write(&fake_byte[1], 1); // MSB first
         bin_output_file.write(&fake_byte[0], 1); // LSB next
         unsigned char io_value = value;
         bin_output_file.write(reinterpret_cast<const char *>(&io_value), 1);
   ...
   while (nextchar(code_byte, bin_source_file)) {
      const char * fake_byte = reinterpret_cast<const char *>(&address);
      if (fake_byte[1] & 0x80) {
         cerr << "Triplet addresses cannot exceed 0x7FFF";
         exit(10);
      }
      // THIS IS NOT PORTABLE!
      bin_output_file.write(&fake_byte[1], 1); // MSB first
      bin_output_file.write(&fake_byte[0], 1); // LSB next
      bin_output_file.write(reinterpret_cast<const char *>(&code_byte), 1);
      ++address;
   }
   ...
   while (command != "__EOF") {
      ...
      if (io_reg == "patch") {
         ...
         unsigned char value2 = static_cast<unsigned char>
            (strtoul(value2_string.c_str(), NULL, 0));

         address = value;
         const char * fake_byte = reinterpret_cast<const char *>(&address);
         bin_output_file.write(&fake_byte[1], 1); // MSB first
         bin_output_file.write(&fake_byte[0], 1); // LSB next
         bin_output_file.write(reinterpret_cast<const char *>(&value2), 1);
      }
      else {
         unsigned short io_address = io_map[io_reg];
         // This is the Rabbit specific way to specify an io register in the
